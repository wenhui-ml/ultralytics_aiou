...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, AIoU=False, eps=1e-7):
    """
    ...
    Args:
        ...
        AIoU (bool, optional): If True, calculate Advanced IoU. Defaults to False.
        eps (float, optional): A small value to avoid division by zero. Defaults to 1e-7.

    Returns:
        (torch.Tensor): IoU, GIoU, DIoU, CIoU, or AIoU values depending on the specified flags.
    """
    ...
    # IoU
    iou = inter / union
    if CIoU or DIoU or GIoU or AIoU:
        cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # convex (smallest enclosing box) width
        ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height
        if CIoU or DIoU or AIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            c2 = cw.pow(2) + ch.pow(2) + eps  # convex diagonal squared
            rho2 = (
                (b2_x1 + b2_x2 - b1_x1 - b1_x2).pow(2) + (b2_y1 + b2_y2 - b1_y1 - b1_y2).pow(2)
            ) / 4  # center dist**2
            if CIoU:  # DIoU-SSD-pytorch/utils/box/box_utils.py at master · Zzh-tju/DIoU-SSD-pytorch
                v = (4 / math.pi**2) * ((w2 / h2).atan() - (w1 / h1).atan()).pow(2)
                with torch.no_grad():
                    alpha = v / (v - iou + (1 + eps))
                return iou - (rho2 / c2 + v * alpha)  # CIoU
            elif AIoU:  # Advanced IoU
                v = (4 / math.pi**2) * ((w2 / h2).atan() - (w1 / h1).atan()).pow(2)

                # Calculate Euclidean distance of the center points of the four edges
                dist_left = (b2_x1 - b1_x1)**2 + ((b2_y1 + b2_y2) / 2 - (b1_y1 + b1_y2) / 2)**2
                dist_right = (b2_x2 - b1_x2)**2 + ((b2_y1 + b2_y2) / 2 - (b1_y1 + b1_y2) / 2)**2
                dist_top = ((b2_x1 + b2_x2) / 2 - (b1_x1 + b1_x2) / 2)**2 + (b2_y1 - b1_y1)**2
                dist_bottom = ((b2_x1 + b2_x2) / 2 - (b1_x1 + b1_x2) / 2)**2 + (b2_y2 - b1_y2)**2

                # Calculate the relative distance of the four edges
                relative_dist_left = dist_left / w2.pow(2)
                relative_dist_right = dist_right / w2.pow(2)
                relative_dist_top = dist_top / h2.pow(2)
                relative_dist_bottom = dist_bottom / h2.pow(2)

                # Calculate the average of relative distances as the distance penalty term
                e = (relative_dist_left + relative_dist_right + relative_dist_top + relative_dist_bottom) / 4
                # # # Use atan function to constrain e in [0, 1]
                e = (2 / math.pi) * e.atan()

                with torch.no_grad():
                    alpha, gamma = v / (v - iou + (1 + eps)), e / (e - iou + (1 + eps))

                # Calculate the median and quartiles of IoU
                iou_mean = iou.mean()
                iou_std = iou.std()

                # Approximate the first and third quartiles
                q1_iou = iou_mean - 0.6745 * iou_std  # Approximate the 25th percentile
                q3_iou = iou_mean + 0.6745 * iou_std  # Approximate the 75th percentile

                # Adjust scale_factor based on the degree of dispersion
                spread = q3_iou - q1_iou  # Degree of dispersion
                scale_factor = (1 - iou_mean) * (10 + spread)  # Adaptive adjustment range in [0, 10]

                # Use hyperbolic tangent function for smoothing
                lambda_gamma = torch.tanh(scale_factor * (0.5 - iou))  # lambda_gamma is larger at low IoU, favoring edge penalty
                lambda_gamma = torch.clamp(lambda_gamma, min=0)
                lambda_alpha = 1 - lambda_gamma             # lambda_alpha is larger at high IoU, favoring aspect ratio penalty
                return iou - (rho2 / c2 + lambda_alpha * alpha * v + lambda_gamma * gamma * e)
